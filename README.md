...
and the package plus cache to `devenv.nix`:
...
Don't set `inputs.nixpkgs.follows` for `dg`: the cache only has builds against
the flake's own pinned nixpkgs, so overriding it compiles `dg` from source. For
the same reason prefer `packages.<system>.default` over `overlays.default`
(which builds `pkgs.dg` against your nixpkgs).
