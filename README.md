...
`nixConfig` offers it on first use (accept the prompt, or pass
...
To use in a [devenv](https://devenv.sh) project, prefer the prebuilt release
binaries: `nix/dg-bin.nix` evaluates in milliseconds against your own nixpkgs
and never compiles. Add the repo as a plain source input in `devenv.yaml`:

```yaml
inputs:
  dg:
    url: github:decisiongraph/dg
    flake: false   # no second nixpkgs, bun2nix etc. in your lock
```
...
Don't set `inputs.nixpkgs.follows` for `dg` here: the cache only has builds
against the flake's own pinned nixpkgs, so overriding it compiles `dg` from
source. For the same reason prefer `packages.<system>.default` over
`overlays.default` (which builds `pkgs.dg` against your nixpkgs).
