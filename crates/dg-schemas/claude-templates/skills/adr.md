...
3. **Create the ADR** (add `--field=value` to set fields at creation):
   ```bash
   dg new adr "Use Ruby on Rails with PostgreSQL" --status=proposed
   ```
...
- Good title: "Use PostgreSQL for primary data store"
